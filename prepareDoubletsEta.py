...
def process_event(prefix, output_dir, pt_min, n_eta_sections, n_phi_sections,
    ...
    feature_names = ['r', 'phi', 'eta']
    feature_scale = np.array([1000., np.pi / n_phi_sections, np.amax([np.absolute(eta_range[0]),np.absolute(eta_range[1])])])
